// FlaskのPython変数から取得（settings変数がテンプレートに渡されていることが前提）
...
if (validData.length === 0) {
    ...
    const chartDataPoints = []; // 👈 全データポイントをここに集約
    const yAxisLabels = []; // Y軸のラベル順序を保持 (表示順)
    ...
    Object.entries(userShifts).forEach(([userId, shifts]) => {
        ...
        shifts.forEach((s) => {
        // X軸を正しく時間オブジェクトとしてパース
        // Chart.jsはDateオブジェクトを期待
    ...
    new Chart(ctx, {
        ...
        datasets: [{
            ...
            backgroundColor: chartDataPoints.map(d => d.backgroundColor), // データポイントごとに色を適用
        ...
        plugins: {
            ...
            callbacks: {
                label: ctx => {
                // ctx.raw は chartDataPoints の要素にアクセスできる
    ...
    // 🚨 補足グラフ: 時間帯別必要人数と実際の人数 (ヒートマップ/ライングラフも有用ですが、ここでは簡単のため省略)
    // 卒業研究では、この下に Line Chart で時間帯の人数超過/不足を表示すると説得力が増します。
    });
    // --- ユーザー別合計勤務時間グラフの追加 ---
...
// 勤務時間と休憩時間を集計
validData.forEach(s => {
if (s.type !== 'work') return; // 休憩時間は含めない
...
const totalHoursData = userLabels.map(userId => totalHoursByUser[userId] / 60); // 時間単位に変換

if (userLabels.length > 0) {
const totalChartContainer = document.createElement("div");
totalChartContainer.classList.add("chart-container", "total-chart");
totalChartContainer.innerHTML = `<h2>⚖️ 全体の勤務時間バランス</h2><canvas id="total_hours_chart"></canvas>`;
...
new Chart(totalCtx, {
    ...
    data: {
        ...
        datasets: [{
            label: "合計勤務時間（時間）",
            data: totalHoursData,
            backgroundColor: 'rgba(54, 162, 235, 0.8)', // 青色系の単色
