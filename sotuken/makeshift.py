...
def auto_calendar():
    ...
    try:
        ...
        # ========================================================
        # 7. 日付ごとの処理（OR-Tools + フォールバック）
        # ========================================================
        base_date = datetime(2000, 1, 1)
        use_ortools = True  # OR-Toolsを使用するかどうかのフラグ
        ...
        for target_date_obj in target_dates:
                ...
                try:
                    ...
                    if not shifts:
                        ...
                        if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
                            ...
                            print(f"DEBUG: OR-Tools求解失敗 (status={status})。貪欲法にフォールバック")
                            use_ortools = False
                
                except Exception as e:
                    print(f"DEBUG: OR-Toolsでエラー発生: {e}")
            ...
            # 11-2. 貪欲法（フォールバック）
            # ========================================================
            if not use_ortools or not assigned_shifts:
                print(f"DEBUG: 貪欲法で処理を開始...")
                assigned_shifts = []
        ...
        # ========================================================
        # 15. 連続シフトのマージ処理（★重要: OR-Toolsの結果も綺麗に表示★）
        # ========================================================
        final_display_shifts = []
        if raw_shifts:
        ...
        algorithm_used = "OR-Tools最適化" if use_ortools else "先着順（貪欲法）"
        message = f"✅ {target_month}月シフト作成完了（{algorithm_used}）: {total_shifts}件 | 🔒保護済み: {locked_shifts}件 | 🚨不足: {shortage_count}件"
...
def reset_demand():
    ...
    try:
        ...
        # デバッグログ
        print(f"🔍 Reset ALL - store_id: {store_id}")
        
        # 削除前のカウント
        cursor.execute("SELECT COUNT(*) as count FROM shift_demand WHERE store_id = %s", (store_id,))
        before_count = cursor.fetchone()["count"]
        print(f"📊 削除前（全体）: {before_count}件")
...
def reset_demand_by_type():
    ...
    try:
        ...
        return jsonify({
            "success": True,
            "message": f"{day_type_label}の設定を{deleted_count}件削除しました",
            "deleted_count": deleted_count
        })
